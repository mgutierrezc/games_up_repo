...
class Constants(BaseConstants):
    ...
    quality_production_costs = dict(High=30, Medium=20, Low=10)
...
class Subsession(BaseSubsession):
    ...
    def vars_for_admin_report(self):
        ...
        series.append({'name': 'Transaction Price', 'data': transaction_prices})

        for player in group.get_players():
            payoffs = [p.payoff for p in player.in_all_rounds()]
            series.append(
                {'name': 'Earnings for {}'.format(player.role()), 'data': payoffs})

        return dict(highcharts_series=series, round_numbers=list(range(1, Constants.num_rounds + 1)))
